...
/* subtract k times row j to row i. */
 ...
 * Just exchange the pointers, not the array values. */
static void EX(double **A, int cols, int i, int j)
{
    double *aux = A[i];
    A[i] = A[j];
    A[j] = aux;
...
/* Scale elements of row i by x */
static void S(double **A, int cols, int i, double x)
{
    int j;
    for (j = 0; j < cols; j++) {
        A[i][j] *= x;
...
/* implements the Gauss/Jordan elimination method on a matrix of
 * rows and cols. */
double
gauss(
        double**            A,
        ...
        const char*         fmt)
{
         ...
         pr++, pc++)
    {
        ...
        for (r = pr + 1; r < rows; r++) {
            double x = A[r][pc] / pv;
            /* subtract x times row pr from row r */
            P(A, cols, r, x, pr);
            mod = TRUE;
        }
        ...
        if ((flags & FLAG_DEBUG) && mod) {
            print_matrix(A, rows, cols, fmt, eps);
        }
    ...
    if (!(flags & FLAG_DONTDOLASTPASS)) {
        while (--p >= pivots) { /* for each pivot */
            ...
            for (r = 0; r < pr; r++) {
                double k = A[r][pc] / pv;
                /* subtract k times row pr from row r */
                P(A, cols, r, k, pr);
                mod = TRUE;
            } /* for r */
